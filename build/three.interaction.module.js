...
const MOUSE_POINTER_ID$1 = 'MOUSE'; // helpers for hitTest() - only used inside hitTest()

const hitTestEvent$1 = {
  target: null,
  data: {
    global: null
  }
...
class InteractionManager extends EventDispatcher {
  ...
  constructor(renderer, scene, camera, options) {
    ...
    this.mouse.identifier = MOUSE_POINTER_ID$1; // setting the mouse to start off far off screen will mean that mouse over does
    ...
    this.activeInteractionData[MOUSE_POINTER_ID$1] = this.mouse;
    /**
  ...
  setCursorMode(mode) {
    ...
    const style = this.cursorStyles[mode]; // only do things if there is a cursor style for it
  ...
  processInteractive(interactionEvent, displayObject, func, hitTest, interactive) {
    ...
    if (displayObject.interactiveChildren && displayObject.children) {
      ...
      for (let i = children.length - 1; i >= 0; i--) {
        const child = children[i]; // time to get recursive.. if this function will return if something is hit..
  ...
  onPointerComplete(originalEvent, cancelled, func) {
    const events = this.normalizeToPointerData(originalEvent);
    const eventLen = events.length; // if the event wasn't targeting our canvas, then consider it to be pointerupoutside
  ...
  processPointerUp(interactionEvent, displayObject, hit) {
    ...
    const isMouse = data.pointerType === 'mouse' || data.pointerType === 'pen'; // Mouse only
  ...
  onPointerOut(originalEvent) {
    // if we support touch events, then only use those for touch events, not pointer events
    if (this.supportsTouchEvents && originalEvent.pointerType === 'touch') return;
    const events = this.normalizeToPointerData(originalEvent); // Only mouse and pointer can call onPointerOut, so events will always be length 1
  ...
  processPointerOverOut(interactionEvent, displayObject, hit) {
    ...
    const id = interactionEvent.data.identifier;
    const isMouse = data.pointerType === 'mouse' || data.pointerType === 'pen';
    let trackingData = displayObject.trackedPointers[id]; // if we just moused over the display object, then we need to track that state
  ...
  onPointerOver(originalEvent) {
    const events = this.normalizeToPointerData(originalEvent); // Only mouse and pointer can call onPointerOver, so events will always be length 1
...
const MOUSE_POINTER_ID = 'MOUSE'; // helpers for hitTest() - only used inside hitTest()
...
export { Interaction, InteractionLayer, InteractionManager };
